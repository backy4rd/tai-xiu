...
module.exports.processBet = (point, betUsers, onlineUsers, io) => {
  betUsers.forEach(({ socketId, username, betInfo: { option, bet } }) => {
    const userInfo = onlineUsers.find(onlineUser => onlineUser.username == username);

    if (this.isWin(option, point)) {
      const betxbase = bet * this.xBase(option);
      userInfo.coin += betxbase;
      io.to(socketId).emit("result", { status: "you win", gain: `+${betxbase} coin` });
    } else {
      userInfo.coin -= bet;
      io.to(socketId).emit("result", { status: "you lose", gain: `-${bet} coin` });
    }
